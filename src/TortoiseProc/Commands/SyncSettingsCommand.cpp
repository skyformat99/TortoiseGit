...
#define TGIT_SYNC_VERSION_STR   L"1"
...
bool SyncSettingsCommand::Execute()
{
    ...
    if (parser.HasKey(L"askforpath"))
    {
        ...
        CString path;
        bool bGotPath = FileOpenSave(path, bWithLocals, !!parser.HasKey(L"load"), hwndExplorer);
        if (bGotPath)
        {
...
bool SyncSettingsCommand::HandleRegistryKey(const CString& regname, CSimpleIni& iniFile, bool bCloudIsNewer)
{
    CAutoRegKey hKey;
    CAutoRegKey hKeyKey;
    DWORD regtype = 0;
